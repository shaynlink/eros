    ...
    ws.onmessage = function (evt) {
        ...
        switch (json.op) {
            ...
            case 5:
                ...
                this.$store.commit('setAuditLogs', json.d.auditlogs);
                break;
            ...
            case 4:
                if (json.t == 'USER_UPDATE') {
                    ...
                    window.localStorage.setItem('user', user);
                } else if(json.t == 'AUDIT_LOG') {
                    if (json.d.action == 'REMOVE') {
                        let arr = this.$store.state.auditlog; 
                        const index = arr.findIndex((v) => v._did == json.d._id);
                        if (index == -1) return;
                        arr = arr.splice(index, 1);
                        this.$store.commit('setAuditLogs', arr);
                    } else this.$store.commit('pushAuditLogs', json.d);
                }
                break;
            default:
                break;
